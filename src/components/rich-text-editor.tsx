...
import { ARTICLE_COMPONENT_OPTIONS, ARTICLE_COMPONENT_WEB_SCRIPT, type ArticleComponentFields } from './rich-text-contract';
...
export function RichTextEditor({ articleComponents = false, minHeight = 320, onChange, placeholder = 'Start typing...', style, value, ...props }: RichTextEditorProps) {
  ...
  const componentOptions = ARTICLE_COMPONENT_OPTIONS.map((option) => `<option value="${option.value}">${option.label}</option>`).join('');
...
</style></head><body><div class="toolbar"><button data-command="undo" title="Undo">↶</button><button data-command="redo" title="Redo">↷</button><button data-command="bold" title="Bold"><b>B</b></button><button data-command="italic" title="Italic"><i>I</i></button><button data-command="underline" title="Underline"><u>U</u></button><button data-align="left" title="Align left">≡</button><button data-align="center" title="Align center">≣</button><button data-align="right" title="Align right">≡</button>${articleComponents ? `<select id="component-type">${componentOptions}</select><button id="insert-component">+ Insert</button>` : ''}</div><div id="editor" class="editor" data-placeholder="${cleanPlaceholder}" contenteditable="true"></div><script>
const initial=${safeJson(initial.current)};const editor=document.getElementById('editor');const labels=${safeJson(labels)};
...
function nodeHtml(node){if(!node||typeof node!=='object')return'';if(node.type==='block')return componentHtml(node.fields);if(node.type==='text')return textHtml(node);const known={paragraph:'p',heading:node.tag||'h2',quote:'blockquote',list:node.tag||(node.listType==='number'?'ol':'ul'),listitem:'li'};const tag=known[node.type];if(!tag)return '<div class="unknown" contenteditable="false" data-preserved="'+encode(node)+'">Unknown content</div>';const metadata={...node};delete metadata.children;const content=(Array.isArray(node.children)?node.children:[]).map(nodeHtml).join('')||'<br>';return '<'+tag+' data-node="'+encode(metadata)+'"'+(metadata.format?' style="text-align:'+esc(metadata.format)+'"':'')+'>'+content+'</'+tag+'>'}
const validInitial=initial&&initial.root&&initial.root.type==='root'&&Array.isArray(initial.root.children);if(initial&&!validInitial){editor.contentEditable='false';editor.innerHTML='<div class="error">Error loading editor</div>'}else if(validInitial){editor.innerHTML=initial.root.children.map(nodeHtml).join('')}
...
</script></body></html>`, [articleComponents, cleanPlaceholder, componentOptions, labels, minHeight, theme.colors.border, theme.colors.card, theme.colors.muted, theme.colors.sea, theme.colors.textPrimary, theme.colors.textSecondary]);
